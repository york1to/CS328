...
void Read_matrix_from_file(char *filename, LOCAL_MATRIX_T *local_A, GRID_INFO_T *grid, int n);
void Print_matrix(char *title, LOCAL_MATRIX_T *local_A, GRID_INFO_T *grid, int n);
void Print_matrix_to_file(char *filename, LOCAL_MATRIX_T *local_A, GRID_INFO_T *grid, int n);
...
void Read_matrix_from_file(char *filename /* in */, LOCAL_MATRIX_T *local_A /* out */, GRID_INFO_T *grid /* in */, int n /* in */)
{
    int mat_row, mat_col, grid_row, grid_col, dest;
    int coords[2];
    float *temp;
    ...
    if (grid->my_rank == 0)
    {
        ...
        for (mat_row = 0; mat_row < n; mat_row++)
        {
            ...
            for (grid_col = 0; grid_col < grid->q; grid_col++)
            {
                ...
                {
                    for (mat_col = 0; mat_col < Order(local_A); mat_col++)
                        fscanf(file, "%f", (local_A->entries) + mat_row * Order(local_A) + mat_col);
                }
                else
                {
                    for (mat_col = 0; mat_col < Order(local_A); mat_col++)
...
void Print_matrix_to_file(char *filename /* in */, LOCAL_MATRIX_T *local_A /* out */, GRID_INFO_T *grid /* in */, int n /* in */)
{
    int mat_row, mat_col, grid_row, grid_col, source;
    int coords[2];
    float *temp;
    ...
    if (grid->my_rank == 0)
    {
        ...
        for (mat_row = 0; mat_row < n; mat_row++)
        {
            ...
            for (grid_col = 0; grid_col < grid->q; grid_col++)
            {
                ...
                {
                    for (mat_col = 0; mat_col < Order(local_A); mat_col++)
                        fprintf(file, "%4.1f ", Entry(local_A, mat_row, mat_col));
                }
                else
                {
                    MPI_Recv(temp, Order(local_A), MPI_FLOAT, source, 0, grid->comm, &status);
                    for (mat_col = 0; mat_col < Order(local_A); mat_col++)
                        fprintf(file, "%4.1f ", temp[mat_col]);
                }
            }
            fprintf(file, "\n");
        }
